...
const InterviewScheduleUI = () => {
  ...
  const scheduleMeeting = async () => {
    ...
    if (!formData.candidateId || formData.interviewerIds.length === 0) {
      toast.error("Please select both candidate and at least one interview");
      return;
    }
    setIsCreating(true);
  ...
  const selectedInterviewers = interviewer.filter((i) =>
    formData.interviewerIds.includes(i.clerkId)
  );

  const availableInterviewers = interviewer.filter(
    (i) => !formData.interviewerIds.includes(i.clerkId)
  );

  return (
    <div className="container max-2-7xl mx-auto p-6 space-y-8">
